...
const { createValidation, deleteValidation, updateValidation } = require("../validators/food_types.js");

router.post("/addFoodType", createValidation, (req, res) => {
    db("tbl_food_types").insert({
...
router.patch("/updateFoodType/:id", updateValidation, createValidation, (req,res) => {
    db("tbl_food_types").where("type_id", req.params.id).update({
        type_name: req.body.type_name
    }).then((data) => {
        return res.json({
...
router.delete("/deleteFoodType/:id", deleteValidation, (req,res) => {
    db("tbl_food_types").where("type_id", req.params.id).del().then((data)=>{ 
        return res.json({
